...
function eventRecord(
  overrides: Partial<CaseTimelineEventRecord> = {},
...
describe("case timeline service", () => {
  it("creates an event for an existing case", async () => {
    ...
    const item = await service.create(
      "case-1",
      { type: "hearing", title: "Audiência inicial", occurredAt: now },
      "user-1",
    );
  ...
  it("blocks events for missing cases", async () => {
    ...
    await expect(
      service.create("missing", { type: "note", title: "Andamento" }, "user-1"),
    ).rejects.toBeInstanceOf(CaseTimelineCaseNotFoundError);
  });
  ...
  it("lists events by most recent occurrence first", async () => {
    ...
    await expect(service.list("case-1")).resolves.toMatchObject([
      { id: "new" },
      { id: "old" },
    ]);
  });
  ...
  it("lists all timeline events across cases", async () => {
    ...
    await expect(service.listAll({ type: "all" })).resolves.toMatchObject([
      { id: "case-2-event" },
      { id: "case-1-event" },
    ]);
  });
