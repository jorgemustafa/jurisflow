...
function handleAuthError(error: unknown, reply: FastifyReply) {
  if (error instanceof ZodError) return reply.code(400).send({ message: "Invalid auth data", issues: error.issues });
  if (error instanceof InvalidCredentialsError) return reply.code(401).send({ message: error.message });
...
export async function authRoutes(app: FastifyInstance) {
  app.post("/login", async (request, reply) => {
    try {
      return authService.login(parseBody(loginSchema, request.body));
    } catch (error) {
