...
import type { ConfirmCaseImportInput, PreviewCaseImportInput } from "./case-import.schemas.js";
...
type CaseImportRepository = {
  ...
  importCase(clientId: string, draft: ImportedCaseDraft): Promise<{ case: CaseRecord; importedMovements: number; skippedMovements: number }>;
};
...
export function createCaseImportService(repository: CaseImportRepository, provider: CaseImportProvider) {
  ...
  return {
    async preview(input: PreviewCaseImportInput) {
      ...
      return { draft: { ...draft, movements: uniqueMovements(draft.movements) }, duplicate: null };
    },
    ...
    async confirm(input: ConfirmCaseImportInput) {
      ...
      const draft = await provider.fetchCase(input);
      return repository.importCase(input.clientId, { ...draft, movements: uniqueMovements(draft.movements) });
    }
  };
}
