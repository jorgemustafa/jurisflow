...
import { DataJudCaseNotFoundError, DataJudConfigError } from "./datajud.client.js";
import { CaseImportClientError, type ImportedCaseDraft, type ImportedMovement } from "./case-import.service.js";
import type { CreateCaseImportBatchInput, UpdateCaseImportItemInput } from "./case-import-batch.schemas.js";
...
export type StoredCaseDraft = Omit<ImportedCaseDraft, "openedAt" | "movements"> & {
...
type CaseImportBatchRepository = {
  ...
  updateItem(
    itemId: string,
    data: Partial<Pick<CaseImportItemRecord, "status" | "clientId" | "caseId" | "errorMessage">>
  ): Promise<void>;
  importItem(itemId: string, clientId: string, draft: ImportedCaseDraft): Promise<{ caseId: string; importedMovements: number; skippedMovements: number }>;
...
export class CaseImportBatchEmptyError extends Error {
  constructor() {
    super("No items are ready to import; link a client to at least one pending item");
  }
}
...
export function serializeDraft(draft: ImportedCaseDraft): StoredCaseDraft {
  return {
    ...
    movements: draft.movements.map((movement) => ({ ...movement, occurredAt: movement.occurredAt.toISOString() }))
  };
}
...
export function parseDraft(draft: StoredCaseDraft): ImportedCaseDraft {
  return {
    ...
    movements: draft.movements.map((movement) => ({ ...movement, occurredAt: new Date(movement.occurredAt) }))
  };
}
...
export function createCaseImportBatchService(
  ...
  async function buildItem(cnjNumber: string): Promise<NewCaseImportItem> {
    ...
    if (existing) {
      return { cnjNumber, courtCode: court.code, status: "duplicate", errorMessage: null, draft: null, caseId: existing.id };
    }

    try {
      const draft = await provider.fetchCase({ cnjNumber, courtCode: court.code });
      return {
        cnjNumber,
        courtCode: court.code,
        status: "pending",
        errorMessage: null,
        draft: serializeDraft({ ...draft, movements: uniqueMovements(draft.movements) }),
        caseId: null
      };
    } catch (error) {
      if (error instanceof DataJudConfigError) throw error;
      if (error instanceof DataJudCaseNotFoundError) {
        return { cnjNumber, courtCode: court.code, status: "failed", errorMessage: "Processo não encontrado no DataJud", draft: null, caseId: null };
      }
      return { cnjNumber, courtCode: court.code, status: "failed", errorMessage: "Falha na consulta ao DataJud", draft: null, caseId: null };
    }
  }
  ...
  return {
    async create(input: CreateCaseImportBatchInput) {
      ...
      const items = await mapWithConcurrency(cnjNumbers, concurrency, buildItem);
    ...
    async updateItem(batchId: string, itemId: string, input: UpdateCaseImportItemInput) {
      const batch = await getOpenBatch(batchId);
      const item = batch.items.find((entry) => entry.id === itemId);
      if (!item) throw new CaseImportItemNotFoundError();

      if (input.status === "discarded") {
        if (item.status !== "pending") throw new CaseImportItemStateError("Only pending items can be discarded");
        await repository.updateItem(itemId, { status: "discarded" });
      }

      if (input.status === "pending") {
        if (item.status !== "discarded") throw new CaseImportItemStateError("Only discarded items can be restored");
        await repository.updateItem(itemId, { status: "pending" });
      }

      if (input.clientId !== undefined) {
        const status = input.status ?? item.status;
        if (status !== "pending") throw new CaseImportItemStateError("Only pending items can be linked to a client");
        ...
        if (input.clientId !== null) {
          ...
          if (client.status !== "active") throw new CaseImportClientError("Client must be active");
        }
    ...
    async confirm(batchId: string) {
      const batch = await getOpenBatch(batchId);
      const ready = batch.items.filter((item) => item.status === "pending" && item.clientId && item.draft);
      ...
      for (const item of ready) {
        ...
        if (existing) {
          await repository.updateItem(item.id, { status: "duplicate", caseId: existing.id });
          duplicates += 1;
          continue;
        }

        const result = await repository.importItem(item.id, item.clientId as string, parseDraft(item.draft as StoredCaseDraft));
        imported += 1;
