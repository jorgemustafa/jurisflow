...
type DbItemStatus = "PENDING" | "DUPLICATE" | "FAILED" | "IMPORTED" | "DISCARDED";

const toApiBatchStatus = (status: DbBatchStatus) => status.toLowerCase() as CaseImportBatchStatus;
const toDbBatchStatus = (status: CaseImportBatchStatus) => status.toUpperCase() as DbBatchStatus;
const toApiItemStatus = (status: DbItemStatus) => status.toLowerCase() as CaseImportItemStatus;
const toDbItemStatus = (status: CaseImportItemStatus) => status.toUpperCase() as DbItemStatus;
...
type DbItemWrite = Partial<Omit<DbItem, "id" | "batchId" | "createdAt" | "updatedAt" | "draft">> & {
...
type Db = {
  caseImportBatch: {
    create(args: { data: { items: { create: DbItemWrite[] } }; include: { items: typeof itemsOrder } }): Promise<DbBatch>;
    findMany(args: { orderBy: { createdAt: "desc" }; take: number; include: { items: typeof itemsOrder } }): Promise<DbBatch[]>;
    findUnique(args: { where: { id: string }; include: { items: typeof itemsOrder } }): Promise<DbBatch | null>;
    update(args: { where: { id: string }; data: { status: DbBatchStatus } }): Promise<DbBatch>;
  };
  caseImportItem: {
    update(args: { where: { id: string }; data: DbItemWrite }): Promise<DbItem>;
  };
  $transaction<T>(fn: (tx: Prisma.TransactionClient & Pick<Db, "caseImportItem">) => Promise<T>): Promise<T>;
};
...
export const caseImportBatchRepository = {
      ...
      data: {
        items: {
          create: items.map((item) => ({
            ...
            draft: item.draft ? (item.draft as unknown as Prisma.InputJsonValue) : undefined,
            caseId: item.caseId
          }))
        }
      },
  ...
  async findBatchById(id: string) {
    const batch = await db.caseImportBatch.findUnique({ where: { id }, include: { items: itemsOrder } });
  ...
  async updateItem(itemId: string, data: Partial<Pick<CaseImportItemRecord, "status" | "clientId" | "caseId" | "errorMessage">>) {
    await db.caseImportItem.update({
      where: { id: itemId },
      data: {
        ...(data.status !== undefined ? { status: toDbItemStatus(data.status) } : {}),
  ...
  async importItem(itemId: string, clientId: string, draft: ImportedCaseDraft) {
    const imported = await db.$transaction(async (tx) => {
      const result = await createCaseWithMovements(tx, clientId, draft);
  ...
  async setBatchStatus(batchId: string, status: CaseImportBatchStatus) {
    await db.caseImportBatch.update({ where: { id: batchId }, data: { status: toDbBatchStatus(status) } });
  }
};
