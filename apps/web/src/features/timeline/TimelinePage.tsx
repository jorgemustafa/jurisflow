    ...
    <>
      ...
      {timeline.data?.length ? (
        <div className="table-wrap">
          <table>
            <thead><tr><th>Tipo</th><th>Andamento</th><th>Cliente</th><th>Processo</th><th>Data</th></tr></thead>
            <tbody>
              {timeline.data.map((event) => (
                <tr key={event.id}>
                  ...
                  <td className="table-text-cell">
                    ...
                    <small>{fieldValue(event.description)} · {event.externalSource === "datajud" ? "DataJud" : event.createdByUserName ?? "Usuário não informado"}</small>
                  </td>
                  <td>{event.clientName ?? "Não informado"}</td>
                  <td><Link className="table-link" to={`/cases/${event.caseId}`}>{event.caseTitle ?? "Ver processo"}</Link></td>
